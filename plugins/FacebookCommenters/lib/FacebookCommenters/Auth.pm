...
sub condition {
    ...
    my $plugin = instance();
    my $blog_id = $blog->id;
    my $facebook_api_key = $plugin->get_config_value('facebook_app_key', "blog:$blog_id");
    my $facebook_api_secret = $plugin->get_config_value('facebook_app_secret', "blog:$blog_id");
    return 1 if $facebook_api_key && $facebook_api_secret;
    $$reason = 
        '<a href="?__mode=cfg_plugins&amp;blog_id=' . $blog->id . '">' .
        $plugin->translate('Set up Facebook Commenters plugin')  .
        '</a>';
    return 0;
}
...
sub __create_return_url {
    ...
    my $return_url = $app->base . $cgi_path . $cfg->CommentScript ."?". join('&', @params);
...
sub login {
    ...
    my $facebook_api_key = instance($app)->get_config_value('facebook_app_key', "blog:$blog_id");

    my $url = "https://www.facebook.com/dialog/oauth?". join('&',
...
sub handle_sign_in {
    ...
    my $url = "https://graph.facebook.com/oauth/access_token?".join('&', @url_params);
...
sub __get_userpic {
    ...
    my $picture_url = "http://graph.facebook.com/".$cmntr->external_id."/picture?type=large";
...
sub plugin_data_pre_save {
    ...
    return 1
        unless ( $obj->plugin eq $PluginKey ) && ( $scope =~ m/^configuration/ );
    ...
    return __check_api_configuration($app, $plugin, $facebook_api_key, $facebook_api_secret);
}
